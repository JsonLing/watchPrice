 ...
 * @param {object} summary summarizePosition 返回值
 * @param {object} levels calcStopLevels 返回值
 * @param {object} [ops] 操作点位 { reduceBelow, reduceAbove, addPoint, firedLabels }
 *   reduceBelow: 下方最近减仓点 {price,label} | null
 *   reduceAbove: 上方最近反弹减仓点 {price,label} | null
 *   addPoint:    加仓点 {price,label} | null
 *   firedLabels: 已触发点位文本数组（显示 ✅ 标记）
 */
export function formatPositionStatus(summary, levels, ops = null) {
  ...
  // 操作点位：下一个减仓点 / 加仓点（放在止损止盈之前，用户更关心）
  const opsLines = [];
  if (ops?.reduceBelow) {
    opsLines.push(`📉 减仓点: ${ops.reduceBelow.price.toFixed(2)} (${ops.reduceBelow.label})`);
  }
  if (ops?.reduceAbove) {
    opsLines.push(`📈 反弹减仓: ${ops.reduceAbove.price.toFixed(2)} (${ops.reduceAbove.label})`);
  }
  if (ops?.addPoint) {
    opsLines.push(`➕ 加仓点: ${ops.addPoint.price.toFixed(2)} (${ops.addPoint.label})`);
  }
  if (ops?.firedLabels?.length) {
    opsLines.push(`✅ 已触发: ${ops.firedLabels.join(' · ')}`);
  }
