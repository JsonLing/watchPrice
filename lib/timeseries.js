...
export function fetchTimeseriesSeries(db, code, options = {}) {
  if (!db) return [];
  const intervalMinutes = Math.max(1, Number(options.intervalMinutes) || TIMESERIES_DEFAULTS.intervalMinutes);
  const limit = Math.max(1, Number(options.limit) || TIMESERIES_DEFAULTS.windowLimit);
  const fetchLimit = Math.max(limit * 6, TIMESERIES_DEFAULTS.minFetch);
  const rows = db
    .prepare(
      `SELECT timestamp, price, high, low, volume FROM price_records WHERE code = ? ORDER BY timestamp DESC LIMIT ?`
    )
    .all(code, fetchLimit);
  return aggregateTimeseries(rows, { intervalMinutes, limit });
}
